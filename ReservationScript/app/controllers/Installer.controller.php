 ...
 */
	function importSQL($file, $admin_username, $admin_password, $prefix)
	{
		...
		foreach ($arr as $v)
		{
			...
			if (!empty($v))
			{
				mysql_query(htmlspecialchars($v)) or die(mysql_error());
			}
		}
 ...
 */
	function step2()
	{
		...
		{
			$_SESSION[$this->default_product]['Install'] = $_POST;
		...
		if (count($match[0]) > 0)
		{
			...
			foreach ($match[2] as $k => $table)
			{
				$r = mysql_query(sprintf("SHOW TABLES FROM `%s` LIKE '%s'",
					$_SESSION[$this->default_product]['Install']['database'],
					$_SESSION[$this->default_product]['Install']['prefix'] . $table
				), $link);
